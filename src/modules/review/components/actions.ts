...
export async function submitReview(review: ReviewSubmission) {
  try {
    ...
    const jwt: any = jwtDecode(headers.authorization.split(" ")[1] || "")
    console.log("Customer ID:", jwt)

    console.log("Headers acction>>>>>>>>:", headers)
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_MEDUSA_BACKEND_URL}/store/review/create`,
      {
        ...
        body: JSON.stringify({
          ...
          customer_id: jwt.app_metadata.customer_id,
        }),
      }
    ).then((res) => res.json())
